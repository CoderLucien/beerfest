...
export async function POST(
  ...
  try {
    ...
    const approval = await adapter.rejectCampaign(
      latest.approvalId,
      body.approver || "管理员",
      body.reason || "未提供理由"
    );
    await adapter.updateCampaign(params.id, { status: "rejected" });
    return NextResponse.json({ data: { approval, campaignStatus: "rejected" } });
  } catch (error) {
